---
id: PokeButtonEditor
title: 'PokeButtonEditor class'
sidebar_label: 'PokeButtonEditor'
description: 'Default framework inspector for PokeButton/api/vrframework-interaction-runtime/PokeButton.'
# Generated by scripts/gen-api.mjs - edit the C# doc comments instead.
---

# PokeButtonEditor

**Class** · namespace `VRFramework.Core.Editors.Defaults` · assembly `VRFramework.Core.Editors` · [view source](https://git.cie-group.cz/vr-framework/vrf4/core/-/blob/interaction/physics-hold/Editor/UI/VRFDefaultEditors.cs#L217)

Default framework inspector for [`PokeButton`](/api/vrframework-interaction-runtime/PokeButton).

```csharp
[CustomEditor(typeof(PokeButton))]
[CanEditMultipleObjects]
public class PokeButtonEditor : VRFDefaultEditor
```

**Inheritance:** [`object`](https://learn.microsoft.com/dotnet/api/system.object) ← [`Object`](https://docs.unity3d.com/ScriptReference/Object.html) ← [`ScriptableObject`](https://docs.unity3d.com/ScriptReference/ScriptableObject.html) ← [`Editor`](https://docs.unity3d.com/ScriptReference/Editor.html) ← [`VRFComponentEditor`](/api/vrframework-core-editors-ui/VRFComponentEditor) ← [`VRFDefaultEditor`](/api/vrframework-core-editors-ui/VRFDefaultEditor) ← `PokeButtonEditor`

